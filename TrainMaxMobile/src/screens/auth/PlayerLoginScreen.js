  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (json.role === 'coach') {
        ...
        setError('🧑‍💼 This is a Coach account. Please use the Coach Login instead.');
        return;
      }
      // genuine player — RootNavigator will route to PlayerApp
    } catch (e) {
  ...
  return (
    <KeyboardAvoidingView style={{ flex:1 }} behavior={Platform.OS==='ios'?'padding':undefined}>
      ...
      <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
        ...
        <View style={s.top}>
          <LinearGradient colors={['#6a11cb','#2575fc']} style={s.circle}>
            <Text style={{ fontSize:32 }}>👤</Text>
          </LinearGradient>
          <Text style={s.title}>Player Login</Text>
          <Text style={s.sub}>Train Max AI — Player Portal</Text>
          <View style={[s.badge,{backgroundColor:'rgba(101,16,203,0.15)',borderColor:'rgba(101,16,203,0.3)'}]}>
            <View style={[s.dot,{backgroundColor:'#818cf8'}]}/>
            <Text style={[s.badgeTxt,{color:'#818cf8'}]}>Player Access</Text>
          </View>
        </View>
        ...
        <View style={s.card}>
          ...
          <View style={s.row}>
            ...
            <TextInput style={s.inp} placeholder="player@example.com" placeholderTextColor={C.textFaint} value={email} onChangeText={setEmail} autoCapitalize="none" keyboardType="email-address" autoCorrect={false}/>
          </View>

          <Text style={s.lbl}>PASSWORD</Text>
          <View style={s.row}>
            <Text style={s.ico}>🔒</Text>
            <TextInput style={[s.inp,{flex:1}]} placeholder="Enter your password" placeholderTextColor={C.textFaint} value={password} onChangeText={setPassword} secureTextEntry={!showPw} autoCapitalize="none"/>
          ...
          <TouchableOpacity onPress={() => setShowForgot(true)} style={{ alignSelf: 'flex-end', marginTop: 10, marginBottom: 4 }}>
            <Text style={{ color: '#818cf8', fontSize: 13, fontWeight: '600' }}>Forgot password?</Text>
          </TouchableOpacity>

          <TouchableOpacity onPress={handleLogin} disabled={loading} activeOpacity={0.85} style={{marginTop:14}}>
            <LinearGradient colors={['#6a11cb','#2575fc']} style={s.btn} start={{x:0,y:0}} end={{x:1,y:0}}>
              {loading ? <ActivityIndicator color="#fff"/> : <Text style={s.btnTxt}>🏆 Sign In as Player</Text>}
            </LinearGradient>
          </TouchableOpacity>

          <View style={s.div}/>
          <Text style={s.sw}>Don't have an account?{' '}<Text style={s.lnk} onPress={()=>navigation.navigate('PlayerSignup')}>Sign Up</Text></Text>
          <Text style={[s.sw,{marginTop:8}]}>Are you a coach?{' '}<Text style={[s.lnk,{color:C.green}]} onPress={()=>navigation.navigate('CoachLogin')}>Coach Login</Text></Text>
